...
void	new_in_redir(char *str, int *i, t_red *new)
{
	if (str[*i + 1] && str[*i + 1] != '<')
	{
		new->name = save_redir_name(str, i);
		new->limiter = NULL;
		new->doub = 0;
		new->in = 1;
	}
	...
	{
		new->limiter = save_redir_name(str, i);
...
t_red	*new_redir(char *str, int *i, t_all *all)
{
	t_red	*new;
...
char	*parse_redirects(char *str, int *i, t_cmd *cmd, t_all *all)
{
	if (str[*i] && str[*i] == '>')
